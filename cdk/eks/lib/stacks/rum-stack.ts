...
export class CloudWatchRumStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: CloudWatchRumStackProps) {
    ...
    const appMonitor = this.createAppMonitor(cognitoIdentity);
    ...
    const putRumEventsPolicy = new PolicyDocument({
      statements: [
        new PolicyStatement({
          ...
          resources: [`arn:aws:rum:${this.props.env?.region}:${this.props.env?.account}:appmonitor/${this.appMonitorName}`],
        }),
      ],
    });
  ...
  private createAppMonitor(cognitoIdentity: CfnIdentityPool): CfnAppMonitor {
    // Create App Monitor
    return new CfnAppMonitor(this, 'PetClinicAppMonitor', {
      ...
      cwLogEnabled: true,
      domain: `*.${this.props.env?.region}.elb.amazonaws.com`,
